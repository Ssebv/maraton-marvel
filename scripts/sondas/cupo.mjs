...
const { cdp, navega, cierra, errores } = await abre({ siembra: {
  'maraton-marvel-tmdb-v11:vigente': entrada(ahora - 2 * dia),
  'maraton-marvel-tmdb-v11:caducada': entrada(ahora - 9 * dia),
  'maraton-marvel-tmdb-v10:vieja': entrada(ahora),
...
try {
  ...
  const poda = await cdp.eval(`['vigente', 'caducada'].map(k => !!localStorage.getItem('maraton-marvel-tmdb-v11:' + k))
    .concat(!!localStorage.getItem('maraton-marvel-tmdb-v10:vieja'), !!localStorage.getItem('maraton-marvel-persona-v3:bio'), !!localStorage.getItem('maraton-marvel-persona-v3:biovieja'))`)
  ...
  const relleno = await cdp.eval(`(() => {
    ...
    for (const tam of [256 * 1024, 16 * 1024, 1024, 64, 1]) {
      ...
      try { for (;;) localStorage.setItem('maraton-marvel-tmdb-v11:relleno' + i++, trozo) } catch {}
    }
    try { localStorage.setItem('maraton-marvel-tmdb-v11:prueba', 'x'.repeat(40)); return -1 } catch { return i }
  })()`)
